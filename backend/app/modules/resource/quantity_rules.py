...
RULE_15_NON_COUNT_UNITS = {"人月", "月", "年", "小时", "天", "%", "GB", "TB", "G"}
...
# 第16条核验部署覆盖；统一配置、共享服务不要求采购套数与主机数相等。
...
class ResourceRuleFinding:
    ...
    evidence_location: dict = field(default_factory=dict)
...
def evaluate_resource_rules(items: list[ParsedResourceItem]) -> list[ResourceRuleFinding]:
    ...
    findings.append(_aggregate_security_findings(check_security_service_quantities(items)))
    findings.extend(check_major_component_quantities(items))
    return findings


def _aggregate_security_findings(details):
    """One rule result; service evidence remains inside that result."""
    from dataclasses import replace
    if len(details) == 1:
        return replace(details[0], resource_name='安全/PaaS/密码关联服务')
    priority = {'不一致': 6, '数量异常': 5, '资料不足': 4, '无法判断': 3, '部分一致': 2, '通过': 1, '不适用': 0}
    selected = max(details, key=lambda d: priority.get(d.result_label, 3))
    return replace(selected, resource_name='安全/PaaS/密码关联服务',
        reason='；'.join(f'{d.resource_name}：{d.result_label}。{d.reason}' for d in details),
        source_quantities={f'{d.resource_name} / {k}': v for d in details for k, v in d.source_quantities.items()},
        row_indexes=list(dict.fromkeys(r for d in details for r in d.row_indexes)),
        evidence_examples=list(dict.fromkeys(e for d in details for e in d.evidence_examples)),
        source_locations=[loc for d in details for loc in d.source_locations],
        source_section='；'.join(dict.fromkeys(d.source_section for d in details)))


def check_security_service_quantities(items: list[ParsedResourceItem]) -> list[ResourceRuleFinding]:
    """Compare shared services without requiring every service in all three lists."""
    grouped = {}
    seen = set()
    for item in items:
        if str(item.unit or '').strip().lower() in {u.lower() for u in RULE_15_NON_COUNT_UNITS}:
            continue
        service = _match_rule_15_service(item)
        if not service:
            continue
        key = (item.source, item.sheet_name, item.row_index, item.name, item.quantity, item.unit, item.raw_text or item.spec)
        if key in seen:
            continue
        seen.add(key)
        bucket = _classify_rule_15_source_bucket(item) or '未识别来源'
        grouped.setdefault(service, []).append((item, bucket))

    if not grouped:
        return [ResourceRuleFinding(rule_code=RULE_15_CODE, rule_name=RULE_15_NAME,
            resource_name='关联服务可比性', severity='需人工确认', result_label='无法判断',
            reason='未识别到可用于关联数量核验的服务项；不能据此认定项目漏列服务或本规则不适用。',
            suggestion='请核实材料是否包含同一服务的关联清单及数量；仅补充实际适用的内容。',
            source_quantities={}, row_indexes=[])]

    findings = []
    for service, records in grouped.items():
        comparable = [(i, b) for i, b in records if b in SOURCE_BUCKET_ALIASES]
        scopes = {}
        for i, b in comparable:
            scopes.setdefault(b, set()).add(i.sheet_name)
        multitable = any(len(v) > 1 for v in scopes.values())
        units = {str(i.unit or '').strip() for i, _ in comparable}
        mixed_units = len(units) != 1 or '' in units
        quantities = {}
        for i, b in comparable:
            key = b + (' / ' + i.sheet_name if multitable else '')
            if mixed_units:
                key += '（' + (i.unit or '单位不明') + '）'
            quantities[key] = quantities.get(key, 0) + i.quantity
        refs = any(any(t in (i.spec + ' ' + i.raw_text) for t in ('详见运维', '另行申报', '历史配置', '原有资源', '原批复', '上期')) for i, _ in comparable)
        stage_sets = {b: {stage for i, bucket in comparable if bucket == b
                          for stage in ('新增', '存量', '原有', '运维', '一期', '二期')
                          if stage in (i.spec + ' ' + i.raw_text)} for b in scopes}
        differing_stages = len({tuple(sorted(v)) for v in stage_sets.values()}) > 1 and any(stage_sets.values())
        label, severity = '无法判断', '需人工确认'
        suggestion = '请说明同一服务的适用清单、项目阶段和计量单位；不要求无关联的清单重复列项。'
        if len(scopes) < 2:
            reason = f'{service}仅在{len(scopes)}类可识别来源中出现，没有足够的关联项可比；单一来源不等于其他清单漏列该服务。'
        elif mixed_units or multitable:
            label = '资料不足'
            reason = f'{service}涉及不同或不明单位、同类多张清单，不能直接相加比较；数量按来源及单位分别保留。'
        else:
            equal = len({round(q, 4) for q in quantities.values()}) == 1
            detail = _format_quantities(quantities)
            if refs or differing_stages:
                label = '资料不足'
                reason = f'{service}在已识别来源中{("数值一致" if equal else "数值不同")}：{detail}；存在运维或历史引用，需确认是否同阶段同范围，不能据此认定缺配。'
            elif not equal:
                label, severity = '不一致', '高'
                reason = f'{service}在已识别的{len(scopes)}类关联清单中数量不一致：{detail}；未识别第三类来源不影响对已存在差异的提示。'
            elif len(scopes) == 3:
                label, severity = '通过', '通过'
                suggestion = None
                reason = f'{service}在已识别的三类关联清单中单位及数量一致；仅对该服务的已识别范围作此结论。'
            else:
                label, severity, suggestion = '通过', '通过', None
                reason = f'{service}在已识别的两类关联清单中单位及数量一致：{detail}；按实际关联项核验通过，不要求第三类无关联清单重复列项。'
        findings.append(ResourceRuleFinding(rule_code=RULE_15_CODE, rule_name=RULE_15_NAME,
            resource_name=service, severity=severity, result_label=label, reason=reason,
            suggestion=suggestion, source_quantities=quantities,
            row_indexes=[i.row_index for i, _ in records],
            evidence_examples=_unique_limited(_item_evidence(i) for i, _ in records),
            source_locations=_item_locations(i for i, _ in records),
            source_section=_join_sections(_item_section(i) for i, _ in records)))
    return findings
...
def check_major_component_quantities(items: list[ParsedResourceItem]) -> list[ResourceRuleFinding]:
    ...
    primary_scope = _rule_16_scope_key(server_matches[0][0]) if server_matches else None
    os_matches = _rule_16_os_matches(items, server_matches)
    db_server_matches = _rule_16_db_server_matches(items, preferred_scope=primary_scope)
    database_matches = _rule_16_database_matches(items, preferred_scope=primary_scope)
    ...
    findings[0].source_locations.extend(_item_locations(i for i, _ in server_matches + os_matches))
    findings[1].source_locations.extend(_item_locations(i for i, _ in db_server_matches + database_matches))
    configured = [(i, q) for i, q in server_matches if _rule_16_server_row_has_os_config(i)]
    standalone = [(i, q) for i, q in os_matches if _is_rule_16_os_product_item(i)]
    if configured and standalone and not _rule_16_os_is_explicitly_additional(standalone):
        ambiguous_indexes.add(0)
        f = findings[0]
        findings[0] = ResourceRuleFinding(**{**f.__dict__,
            'source_quantities': {**f.source_quantities, '资源表已填写系统的服务器数量': sum(q for _, q in configured), '独立操作系统授权数量': sum(q for _, q in standalone)},
            'result_label': '无法判断', 'severity': '需人工确认',
            'reason': '资源表已填写系统的服务器数量与独立授权数量分别保留；二者是否重复覆盖尚未确认，未直接相加。',
            'evidence_examples': _match_evidence(server_matches + os_matches)})
        if server_total > 0 and sum(q for _, q in configured) >= server_total:
            f = findings[0]
            findings[0] = ResourceRuleFinding(**{**f.__dict__, 'result_label': '通过', 'severity': '通过', 'suggestion': None,
                'reason': '已识别服务器逐行填写了操作系统配置，部署覆盖关系完整；独立授权清单不与已配置数量重复累加，也不要求采购套数与主机数相等。'})
    alternate_scopes = sorted({_rule_16_scope_key(item) for item in items
        if _rule_16_has_detailed_table_scope(item)
        and _is_rule_16_server_resource_item(item)
        and not _is_rule_16_spec_only_item(item)
        and not _is_rule_16_budget_or_quote_row(item)
        and _rule_16_scope_key(item) != primary_scope})
    if alternate_scopes:
        ambiguous_indexes.update((0, 1))
    virtualization = any('虚拟化' in _rule_16_full_text(i) and any(t in _rule_16_full_text(i) for t in ('许可', '每cpu', '授权')) for i in items)
    if virtualization:
        ambiguous_indexes.add(0)
    for index, matches in enumerate((server_matches + os_matches, db_server_matches + database_matches)):
        if any(any(t in _rule_16_full_text(item) for t in ("详见运维", "历史配置", "原有资源", "另行申报")) for item, _ in matches):
            ambiguous_indexes.add(index)
    if _rule_16_has_many_to_many_context(db_server_matches):
        ambiguous_indexes.add(1)
    if _rule_16_has_many_to_many_context(server_matches) and not all(
        _rule_16_server_row_has_os_config(item) for item, _quantity in server_matches
    ):
        ambiguous_indexes.add(0)
    for index in ambiguous_indexes:
        finding = findings[index]
        # A second table or cluster does not invalidate a verified equality.
        if finding.result_label == '通过':
            findings[index] = ResourceRuleFinding(**{**finding.__dict__,
                'reason': finding.reason + '已识别可比范围未发现明显数量错误；其他资源表及复用关系不作为重复新增累加。'})
        elif finding.result_label == '不一致':
            configured_db = bool(db_server_matches) and all(_rule_16_server_row_has_db_config(i) for i, _ in db_server_matches)
            if index == 1 and configured_db and _rule_16_has_many_to_many_context(db_server_matches) and db_server_total > 0:
                findings[index] = ResourceRuleFinding(**{**finding.__dict__, 'severity': '通过', 'result_label': '通过',
                    'reason': f'已识别的{db_server_total:g}台数据库服务器均填写了数据库配置，且注明集群部署；另表数据库数量为{database_total:g}。部署配置与采购授权不要求逐台等量，已识别范围未发现明显配置数量错误；不据此认定所有商业授权覆盖均已核实。', 'suggestion': None})
            else:
                findings[index] = ResourceRuleFinding(**{**finding.__dict__, 'severity': '需人工确认', 'result_label': '无法判断',
                    'reason': ('识别到物理服务器及虚拟化许可，物理主机数不能直接与虚拟机操作系统套数比较；尚缺可核验的虚拟机部署覆盖关系。' if index == 0 and virtualization else finding.reason + '采购、预装、复用或部署范围尚未明确；此结果不认定缺配或超配。'),
                    'suggestion': '如需进一步核验，请说明已有资源与新增授权覆盖范围。'})
    # An unrelated table alone is not an error, but explicit missing configuration
    # must not disappear when the preferred table happens to balance.
    for index, field in enumerate(('操作系统', '数据库')):
        missing = [i for i in items if _is_rule_16_server_resource_item(i)
                   and not _is_rule_16_budget_or_quote_row(i)
                   and re.search(rf'{field}\s*[：:]\s*(?:未配置|未配备|缺少|待配置)', i.spec)]
        if missing and findings[index].result_label == '通过':
            f = findings[index]
            f.source_locations.extend(_item_locations(missing))
            findings[index] = ResourceRuleFinding(**{**f.__dict__,
                'result_label': '无法判断', 'severity': '需人工确认',
                'reason': f'部分资源行明确标注{field}未配置或待配置，需核实是否由已有或另行采购的资源覆盖；不能仅凭主表数值相等确认全部已配置。',
                'suggestion': '核实明确未配置行的实际覆盖安排。',
                'evidence_examples': _unique_limited(f.evidence_examples + [_item_evidence(i) for i in missing])})
    return findings


def _rule_16_server_row_has_db_config(item: ParsedResourceItem) -> bool:
    """Use actual configuration cells, not server names or compatibility prose."""
    for cell in item.spec.split('|'):
        pair = re.split(r'[：:]', cell.strip(), maxsplit=1)
        if len(pair) != 2 or pair[0].strip() not in {'数据库', '数据库软件', '单台配置', '软件配置'}:
            continue
        value = pair[1].strip()
        if re.search(r'未|待|无需|不配|不含|缺少|兼容|支持|要求|按需|另行', value):
            continue
        if re.fullmatch(r'[\d\s./\\—-]+|无|不涉及|数据库|是|否', value, flags=re.I):
            continue
        if re.search(r'数据库|OLAP|OLTP|Kingbase|Oracle|PostgreSQL|MySQL|SQL\s*Server|达梦|人大金仓|Gauss|OceanBase', value, re.I):
            return True
    return False
...
def _compare_quantity(
    ...
    if expected <= 0 and actual <= 0:
        return ResourceRuleFinding(
            ...
            severity="需人工确认",
            result_label="无法判断",
            reason=f"未识别到{resource_name}相关的可信资源数量；提取为空不能证明本条不适用。",
...
def _rule_16_server_matches(items: Iterable[ParsedResourceItem]) -> list[tuple[ParsedResourceItem, float]]:
    ...
    for item in items:
        ...
        if _is_rule_16_budget_or_quote_row(item):
            ...
            fallback.append(match)
    # 带操作系统字段的行优先作为“主清单”，但不能因此丢弃同一资源表中
    # 其它未填写操作系统字段的服务器行。报价/预算行仅在没有资源清单行时兜底，
    # 避免“部分服务器带 OS、部分服务器未带 OS”时左侧数量被低估。
    non_quote_fallback = [match for match in fallback if not _is_rule_16_budget_or_quote_row(match[0])]
    candidates = [*structured_with_os, *non_quote_fallback]
    if not candidates:
        candidates = fallback
    candidates = _dedupe_rule_16_quantity_matches(candidates)
    detailed_candidates = [match for match in candidates if _rule_16_has_detailed_table_scope(match[0])]
    if detailed_candidates:
        candidates = [
            match
            for match in candidates
            if not _rule_16_is_narrative_summary(match[0])
        ]
    return _rule_16_select_primary_scope(candidates)
...
def _rule_16_os_matches(
    ...
    standalone = _rule_16_select_primary_scope(_dedupe_rule_16_quantity_matches(matches), preferred_scope=_rule_16_scope_key(server_matches[0][0]) if server_matches else None)
    if not covered_server_rows:
        return standalone

    if not standalone:
        return covered_server_rows
    if _rule_16_os_is_explicitly_additional(standalone):
        return [*covered_server_rows, *standalone]
    # An independent license line may already cover the preinstalled rows.
    return standalone


def _rule_16_os_is_explicitly_additional(matches):
    return bool(matches) and all(any(t in (i.spec + ' ' + i.raw_text) for t in
        ('用于未配置', '用于剩余', '仅覆盖剩余', '不含预装', '不包含预装', '不含已配')) for i, _ in matches)



def _rule_16_db_server_matches(
    items: Iterable[ParsedResourceItem],
    ...
    matches = _dedupe_rule_16_quantity_matches(structured or fallback)
    return _rule_16_select_primary_scope(matches, preferred_scope=preferred_scope)


def _rule_16_database_matches(
    items: Iterable[ParsedResourceItem],
    preferred_scope: str | None = None,
) -> list[tuple[ParsedResourceItem, float]]:
    ...
    # 同一数据库产品在多张清单中的精确重复由去重函数处理；不同数据库产品
    # 不能只取最大值，否则 Oracle 2 + PostgreSQL 1 会被误判为 2 而不是 3。
    matches = _dedupe_rule_16_quantity_matches(purchase_matches or fallback_matches)
    return _rule_16_select_primary_scope(matches, preferred_scope=preferred_scope)


def _rule_16_scope_key(item: ParsedResourceItem) -> str:
    """Return the smallest stable document scope used to avoid cross-table totals."""

    return str(item.sheet_name or item.source or "").strip()


def _rule_16_scope_sort_key(scope: str) -> tuple[int, str]:
    match = re.search(r"(\d+)$", scope)
    return (int(match.group(1)) if match else 10**9, scope)


def _rule_16_has_detailed_table_scope(item: ParsedResourceItem) -> bool:
    return str(item.sheet_name or "").lower().startswith("docx") and "表格" in str(item.sheet_name or "")


def _rule_16_is_narrative_summary(item: ParsedResourceItem) -> bool:
    text = _rule_16_full_text(item)
    return (
        str(item.sheet_name or "").lower() == "docx正文"
        and any(token in text for token in ("\u4e91\u4e3b\u673a", "\u5b50\u7cfb\u7edf", "\u5206\u522b\u4f5c\u4e3a", "\u7ea6"))
    )


def _rule_16_select_primary_scope(
    matches: list[tuple[ParsedResourceItem, float]],
    preferred_scope: str | None = None,
) -> list[tuple[ParsedResourceItem, float]]:
    """Select one detailed table scope instead of summing alternate tables and prose."""

    if not matches:
        return []
    if preferred_scope:
        preferred = [match for match in matches if _rule_16_scope_key(match[0]) == preferred_scope]
        if preferred:
            return preferred
    grouped: dict[str, list[tuple[ParsedResourceItem, float]]] = {}
    for match in matches:
        grouped.setdefault(_rule_16_scope_key(match[0]), []).append(match)
    ranked = sorted(
        grouped.items(),
        key=lambda item: (
            sum(1 for match in item[1] if _rule_16_server_row_has_os_config(match[0])),
            len(item[1]),
            sum(match[1] for match in item[1]),
            tuple(-value for value in _rule_16_scope_sort_key(item[0])[:1]),
        ),
        reverse=True,
    )
    return ranked[0][1]
...
def _rule_16_has_many_to_many_context(matches: list[tuple[ParsedResourceItem, float]]) -> bool:
    ambiguity_keywords = (
        "复用",
        "利旧",
        "主备",
    ...
    return any(
        _has_any_lower(_rule_16_full_text(item), ambiguity_keywords)
        for item, _quantity in matches
    )
...
def _is_rule_16_server_resource_item(item: ParsedResourceItem) -> bool:
    ...
    if _has_any_lower(
        ...
        return False
    # "服务器系统/服务器软件" are software rows.  Treating the substring
    # "服务器" as hardware creates false server totals in mixed equipment lists.
    if any(token in name for token in ("服务器系统", "服务器软件", "服务器操作系统")):
        return False
    if '虚拟化' in full and any(t in full for t in ('许可', '每cpu', '授权')):
        return False
    if _has_any_lower(name, RULE_16_SERVER_EXCLUDE_KEYWORDS):
        return False
    if re.search(r"资源类型[：:]\s*(?:虚拟机|物理机|云主机|裸金属)", item.spec):
        return True
...
def _is_rule_16_database_server_resource_item(item: ParsedResourceItem) -> bool:
    ...
    name = _rule_16_name_text(item)
    name_and_spec = _rule_16_text(item)
    full = _rule_16_full_text(item)
    if _is_rule_16_database_product_item(item):
        return False
    if re.search(r"资源类型[：:]\s*(?:虚拟机|物理机|云主机|裸金属)", item.spec) and any(t in name for t in ("数据库", "前置库")):
        return True
    # A prose sentence may mention both application and database servers.  Only
    # an explicit name/spec field is strong enough to classify the row as a DB server.
    if _has_any_lower(name_and_spec, DATABASE_SERVER_ALIASES):
        return True
...
def _is_rule_16_os_product_item(item: ParsedResourceItem) -> bool:
    ...
    name = _rule_16_name_text(item)
    full = _rule_16_full_text(item)
    if '服务器系统' in name and any(t in full for t in ('麒麟', 'kylin', '统信', 'windows', 'linux')):
        return True
...
def _is_rule_16_database_product_item(item: ParsedResourceItem) -> bool:
    ...
    name = _rule_16_name_text(item)
    full = _rule_16_full_text(item)
    if _has_any_lower(name, RULE_16_DATABASE_EXCLUDE_KEYWORDS):
        return False
...
def _dedupe_rule_16_quantity_matches(
    ...
    for item, quantity in matches:
        strict_key = (f"{item.sheet_name}:{item.row_index}:" + _normalize_rule_16_identity(f"{item.name} {item.spec}"), round(quantity, 4))
...
def _rule_16_effective_quantity(item: ParsedResourceItem, prefer_coverage_quantity: bool) -> float:
    ...
    # Performance limits (e.g. tested on eight nodes) are not purchased licenses.
    text = f"{item.spec} {item.raw_text}"
    matches = re.findall(r'(?:授权|许可|覆盖)(?:数量)?\s*[：:]?\s*(\d+(?:\.\d+)?)\s*(?:台|套|节点|实例|个)', text)
    coverage_quantity = max(map(float, matches)) if matches else None
...
def _item_locations(items):
    """Keep table identity with each matched row; row numbers alone collide."""
    return [dict(sheet_name=item.sheet_name, row_index=item.row_index,
                 source=item.source, quote=item.raw_text or item.spec, precision='row')
            for item in items]
