...
for /f "tokens=2,*" %%A in ('reg query HKCU\Environment /v DEEPSEEK_API_URL 2^>nul') do set "DEEPSEEK_API_URL=%%B"
for /f "tokens=2,*" %%A in ('reg query HKCU\Environment /v DEEPSEEK_API_BASE_URL 2^>nul') do set "DEEPSEEK_API_BASE_URL=%%B"
for /f "tokens=2,*" %%A in ('reg query HKCU\Environment /v DEEPSEEK_MODEL 2^>nul') do set "DEEPSEEK_MODEL=%%B"
for /f "tokens=2,*" %%A in ('reg query HKCU\Environment /v LLM_API_KEY 2^>nul') do set "LLM_API_KEY=%%B"
for /f "tokens=2,*" %%A in ('reg query HKCU\Environment /v LLM_BASE_URL 2^>nul') do set "LLM_BASE_URL=%%B"
for /f "tokens=2,*" %%A in ('reg query HKCU\Environment /v LLM_MODEL 2^>nul') do set "LLM_MODEL=%%B"
...
set "NO_PROXY=llmapi.tongji.edu.cn,192.168.174.218,localhost,127.0.0.1,%NO_PROXY%"
